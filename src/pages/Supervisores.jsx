...
const empty = { nombre: '', email: '', telefono: '', activo: true }
...
export default function Supervisores() {
  ...
  async function fetchAll() {
    setLoading(true)
    const { data } = await supabase.from('supervisores').select('*').order('nombre')
    setRows(data ?? [])
    setLoading(false)
  }
  ...
  async function handleSave() {
    ...
    const { error } = form.id
      ? await supabase.from('supervisores').update(payload).eq('id', form.id)
      : await supabase.from('supervisores').insert(payload)
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-slate-800">Supervisores</h1>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-100">
        ...
        {loading ? (
          ...
          <div className="p-8 text-center text-slate-400 text-sm">
            {search ? 'Sin resultados' : 'No hay supervisores cargados aún'}
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-slate-100">
                  ...
                  <th className="text-left px-4 py-3 text-xs font-medium text-slate-500 uppercase tracking-wide hidden sm:table-cell">Teléfono</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {filtered.map(row => (
                  <tr key={row.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-slate-500 hidden sm:table-cell">{row.telefono || '—'}</td>
                    <td className="px-4 py-3 text-slate-500 hidden md:table-cell">{row.email || '—'}</td>
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}
        title={form.id ? 'Editar supervisor' : 'Nuevo supervisor'}>
